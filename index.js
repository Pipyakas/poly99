...
// end include: /tmp/tmpl1yeqapf.js
// include: /tmp/tmp_5f127za.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmp_5f127za.js
// include: /tmp/tmpndvuhps7.js

    if (!Module['preRun']) throw 'Module.preRun should exist because file support used it; did a pre-js delete it?';
    necessaryPreJSTasks.forEach((task) => {
      if (Module['preRun'].indexOf(task) < 0) throw 'All preRun tasks that exist before user pre-js code should remain after; did you replace Module or modify Module.preRun?';
    });
  // end include: /tmp/tmpndvuhps7.js
...
var ASM_CONSTS = {
  101564: () => { return window.innerWidth; },  
 101594: () => { return window.innerHeight; },  
 101625: () => { if (typeof localStorage === 'undefined') return -1; var s = localStorage.getItem('poly99_touch'); if (s === null || s === '') return -1; return parseInt(s, 10) ? 1 : 0; },  
 101798: () => { var ua = navigator.userAgent || ''; if (navigator.maxTouchPoints > 0 || ('ontouchstart' in window) || (ua.indexOf('Android') >= 0)) return 1; return 0; },  
 101954: ($0) => { if (typeof localStorage !== 'undefined') { localStorage.setItem('poly99_touch', $0 ? '1' : '0'); } },  
 102057: ($0, $1) => { if (typeof localStorage === 'undefined') return $1; var v = localStorage.getItem(UTF8ToString($0)); return (v === null || v === '') ? $1 : parseFloat(v); },  
 102215: ($0, $1) => { if (typeof localStorage === 'undefined') return $1; var v = localStorage.getItem(UTF8ToString($0)); return (v === null || v === '') ? $1 : parseInt(v, 10); },  
 102375: ($0, $1) => { if (typeof localStorage !== 'undefined') localStorage.setItem(UTF8ToString($0), $1.toString()); },  
 102475: ($0, $1) => { if (typeof localStorage !== 'undefined') localStorage.setItem(UTF8ToString($0), $1.toString()); },  
 102575: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; let miniaudio = window.miniaudio; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === miniaudio.device_state.started) { device.webaudio.resume().then(() => { _ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 104753: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, window.miniaudio.unlock, true); }); window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 105057: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 105161: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 108209: ($0) => { return window.miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 108282: ($0) => { var device = window.miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 108682: ($0) => { window.miniaudio.untrack_device_by_index($0); },  
 108732: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = window.miniaudio.device_state.started; },  
 108871: ($0) => { var device = window.miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = window.miniaudio.device_state.stopped; },  
 109011: () => { if (document.fullscreenElement) return 1; },  
 109057: () => { return Module.canvas.width; },  
 109089: () => { return parseInt(Module.canvas.style.width); },  
 109137: () => { document.exitFullscreen(); },  
 109164: () => { setTimeout(function(){ Module.requestFullscreen(false, false); }, 100); },  
 109236: () => { if (document.fullscreenElement) return 1; },  
 109282: () => { return Module.canvas.width; },  
 109314: () => { return screen.width; },  
 109339: () => { document.exitFullscreen(); },  
 109366: ($0) => { const canvasId = UTF8ToString($0); setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { document.querySelector(canvasId).style.width="unset"; }, 100); }, 100); },  
 109560: () => { return window.innerWidth; },  
 109586: () => { return window.innerHeight; },  
 109613: () => { if (document.fullscreenElement) return 1; },  
 109659: () => { return Module.canvas.width; },  
 109691: () => { return parseInt(Module.canvas.style.width); },  
 109739: () => { if (document.fullscreenElement) return 1; },  
 109785: () => { return Module.canvas.width; },  
 109817: () => { return screen.width; },  
 109842: () => { return window.innerWidth; },  
 109868: () => { return window.innerHeight; },  
 109895: () => { if (document.fullscreenElement) return 1; },  
 109941: () => { return Module.canvas.width; },  
 109973: () => { return screen.width; },  
 109998: () => { document.exitFullscreen(); },  
 110025: () => { if (document.fullscreenElement) return 1; },  
 110071: () => { return Module.canvas.width; },  
 110103: () => { return parseInt(Module.canvas.style.width); },  
 110151: () => { document.exitFullscreen(); },  
 110178: ($0) => { Module.canvas.style.opacity = $0; },  
 110216: () => { return screen.width; },  
 110241: () => { return screen.height; },  
 110267: () => { return window.screenX; },  
 110294: () => { return window.screenY; },  
 110321: () => { return window.devicePixelRatio; },  
 110357: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 110410: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 110461: () => { Module.canvas.style.cursor = 'none'; },  
 110498: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 110754: ($0) => { Module.canvas.style.cursor = UTF8ToString($0); },  
 110805: () => { if (document.pointerLockElement) return 1; },  
 110852: () => { if (document.fullscreenElement) return 1; },  
 110898: () => { return window.innerWidth; },  
 110924: () => { return window.innerHeight; }
};
